...
func buildIndex(
	name string, fields []field.Field, vectorDim int, textSearchEnabled bool, hnsw HNSWConfig,
	...
	// TEXT field for BM25 keyword search (only if backend supports it)
	if textSearchEnabled {
		def.Fields = append(def.Fields, db.IndexField{
			Name:  "$.__content",
			Alias: "__content",
	...
	// Vector field — always HNSW, COSINE
	def.Fields = append(def.Fields, db.IndexField{
		Name:              "$.__vector",
		Alias:             "vector",
